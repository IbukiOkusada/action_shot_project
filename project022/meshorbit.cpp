...
void CMeshOrbit::Update(void)
{
	...
	{
		//CManager::GetDebugProc()->Print("[%d] [%f, %f, %f]\n", nCntWidth, m_pVtx[nCntWidth].pos.x, m_pVtx[nCntWidth].pos.y, m_pVtx[nCntWidth].pos.z);
	}
}
...
void CMeshOrbit::SetBullet(void)
{
	...
	{// 親がいる
	//最新の情報を代入
		m_pPos[0].x = m_pMtxParent->_41 + cosf((m_rot.z + D3DX_PI * 1.0f) * -m_nor.z) * m_fRadius;
		m_pPos[0].y = m_pMtxParent->_42 + sinf(m_rot.z + D3DX_PI * 1.0f + (m_nor.y * D3DX_PI)) * m_fRadius;
		m_pPos[0].z = m_pMtxParent->_43 + cosf((m_rot.z + D3DX_PI * 1.0f) * m_nor.x) * m_fRadius;
		...
		m_pPos[MAX_WIDTH].x = m_pMtxParent->_41 + cosf((m_rot.z + D3DX_PI * 0.0f) * -m_nor.z) * m_fRadius;
		m_pPos[MAX_WIDTH].y = m_pMtxParent->_42 + sinf(m_rot.z + D3DX_PI * 0.0f + (m_nor.y * D3DX_PI)) * m_fRadius;
		m_pPos[MAX_WIDTH].z = m_pMtxParent->_43 + cosf((m_rot.z + D3DX_PI * 0.0f) * m_nor.x) * m_fRadius;
