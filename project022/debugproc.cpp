...
void CDebugProc::Update(void)
{
	...
	CManager::GetDebugProc()->Print("アイテムの総数 : %d\n", CObject::GetItemNumAll());
	CManager::GetDebugProc()->Print("優先順位別(%d個分) ↓\n", NUM_PRIORITY);
	for (int nCntPri = 0; nCntPri < NUM_PRIORITY; nCntPri++)
	{
		CManager::GetDebugProc()->Print("[%d] : %d  ", nCntPri, CObject::GetPriNumAll(nCntPri));
	...
	CManager::GetDebugProc()->Print("\n");
	CManager::GetDebugProc()->Print("テクスチャの読み込み総数(%d) \n", CManager::GetTexture()->GetNumAll());
	CManager::GetDebugProc()->Print("モデルの読み込み総数    (%d) \n", CManager::GetModelFile()->GetNumAll());
}
